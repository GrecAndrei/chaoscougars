...
// External trigger system integration
...
#undef INIT_COMPONENT
	...
	// Initialize external trigger system
	EffectDatabase::InitializeRegistry();
	
	// Initialize external trigger system with default config
	bool externalEnabled = g_OptionsManager.GetConfigValue({ "ExternalTrigger_Enabled" }, true); // Default to true for Cougar Journey
	
	if (externalEnabled)
	{
		int externalPort = g_OptionsManager.GetConfigValue({ "ExternalTrigger_Port" }, 8080);
		ExternalTrigger::Initialize(externalPort);
		LOG("External trigger system enabled on port " << externalPort);
	}
	else
	{
		LOG("External trigger system disabled (set ExternalTrigger_Enabled=true in config to enable)");
	}
...
namespace Main
{
	...
	void OnCleanup()
	{
		...
		// Shutdown external trigger system FIRST
